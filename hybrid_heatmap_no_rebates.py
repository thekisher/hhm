...
class HybridHeatmapNoRebates:
    ...
    def __init__(self, initial_capital=50.0, live_mode=True):
        # Capital management
        ...
        # Live trading mode (signals only, no internal execution)
        ...
        if self.live_mode:
            print("🔴 LIVE MODE: Signals only, no internal trading")
        print("=" * 60)
    ...
    def analyze_trading_signal(self, current_price: float, timestamp: float) -> Optional[Dict]:
        ...
        # Skip if already in position (only in simulation mode)
        ...
        if signal:
            if not self.live_mode:
                # Only execute internally if not in live mode
                self.execute_trade(signal)
            # In live mode, just return the signal for external execution
        
        return signal
    
    def execute_trade(self, signal: Dict):
        """Execute trade with enhanced position sizing (simulation mode only)"""
        if self.live_mode:
            # In live mode, don't execute internally
            return
    ...
    def check_exit_conditions(self, current_price: float, timestamp: float) -> Optional[Dict]:
        ...
        if self.live_mode:
            # In live mode, exit conditions are handled by the trader
            return None
    ...
    def close_position(self, exit_price: float, timestamp: float, reason: str, net_pnl: float):
        """Close position with enhanced tracking (simulation mode only)"""
        if self.live_mode:
            # In live mode, don't close internally
            return None
